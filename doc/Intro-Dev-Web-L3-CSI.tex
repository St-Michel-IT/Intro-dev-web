...
\begin{document}
    ...
    \begin{frame}{Introduction au Développement Web}{Compétences}
        ...
        \begin{itemize}
            \item Introduction aux Frameworks Back-end
            \item Présentation des Frameworks Back-end
            \item Avantages et inconvénients des Frameworks Back-end
            \item Choix du framework en fonction des besoins du projet
            \item Comparaison entre différents Frameworks Symfony, Laravel, Django, Spring Boot\ldots
            \item Installation et configuration du framework choisi
        \end{itemize}
        \bigbreak
        Des remarques~?
    \end{frame}
    ...
    \begin{frame}{Les ressources du Web}{Division en briques}
        \begin{columns}
            \column{0.6\textwidth}
    ...
    \begin{frame}{Les ressources du Web}{Analyse des composants, exercice \execcounterdispinc{}}
        Listez toutes les ressources (parties prenantes) du Web que vous connaissez.
        \begin{itemize}
            \item ...
        \end{itemize}
        \bigbreak
    ...
    \begin{frame}{Front ou back, Wooclap NUDMYH)}{Next.JS}
        \centering
        \includegraphics[width=8cm]{image/wooclap-NUDMYH}
    \end{frame}

    \begin{frame}{Front ou back, Wooclap NUDMYH}{React de Facebook}
        \centering
        \includegraphics[width=8cm]{image/wooclap-NUDMYH}
    \end{frame}

    \begin{frame}{Front ou back, Wooclap NUDMYH}{Express (Node JS)}
        \centering
        \includegraphics[width=8cm]{image/wooclap-NUDMYH}
    \end{frame}

    \begin{frame}{Front ou back, Wooclap NUDMYH}{Vue.JS}
        \centering
        \includegraphics[width=8cm]{image/wooclap-NUDMYH}
    \end{frame}

    \begin{frame}{Front ou back, Wooclap NUDMYH}
        \href{https://app.wooclap.com/events/NUDMYH/questions/68d2fccb2cb7398e024e072b}{Énumérer les différences entre backend et frontend~?}
        \bigbreak
        \centering
        \includegraphics[width=5cm]{image/two-side-mirror}
    \end{frame}
    ...
    \begin{frame}{Les protocoles}{Introductions\footnote{\label{sendbird-protocole}Protocoles de communication WebSocket vs. HTTP, \url{https://sendbird.com/fr/developer/tutorials/websocket-vs-http-communication-protocols}}}
        Si on définit le Web comme les applications communicants entre un navigateur et un serveur Web.
        ...
        \end{dangercolorbox}
        \href{https://app.wooclap.com/events/NUDMYH/questions/68d2fb6cbbf6a5629eb98f90}{Quels sont les protocoles qui permettent à ces applications de communiquer~?}
        \pause
        \bigbreak
        Il n'y en a que 2~!
        \begin{itemize}
    ...
    \begin{frame}{Les protocoles}{Websocket, exercice \execcounterdispinc{}}
        ...
        \begin{itemize}
            ...
            \item Développer un serveur backend WebSocket avec la techno/librairie de votre choix.
            Ce serveur va servir l'heure au frontend sans demande sa part.
        ...
        \begin{dangercolorbox}
            ...
            Est-il nécessaire d'utiliser des librairies côté frontend (dépendances logicielles)~?
        \end{dangercolorbox}
    \end{frame}
    ...
    \begin{frame}{HTTP}{Introduction à Content-Type\footnote{\label{mozilla-content-type}Content-Type, \url{https://developer.mozilla.org/fr/docs/Web/HTTP/Headers/Content-Type}}}
        \href{https://app.wooclap.com/events/NUDMYH/questions/68d2fc6907003967e10f34dd}{Quelles types de documents~?}
        \pause
        \begin{itemize}
...
export default App;
    ...
    \begin{frame}{Les Frameworks}{Framework back-end}
        ...
        Le moteur de templating permet un meilleur SEO (référencement naturel) qu'une UI générée \textit{client-side} car le rendu est plus rapide.
        \bigbreak
        Parmi les plus populaires~:
        \begin{itemize}
    ...
    \begin{frame}{Les Frameworks}{Les différences Flask et Django}
        \href{https://app.wooclap.com/events/NUDMYH/questions/68d2fd2007003967e11025cb}{Les intérêts d'un ORM à part VS Django~?}
        \pause
        \begin{itemize}
            \item \textbf{Django} est un framework complet, avec ORM (Django ORM), templating (Django Template Language (DTL), authentification, panneau d'administration, \textit{etc}.
            \item \textbf{Flask} est un micro-framework, minimaliste, qui laisse le choix des composants à l'utilisateur.
            \item Les deux ont des plugins pour être complétés.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Les Frameworks}{Framework back-end, exercices}
        ...
        \begin{columns}
            ...
            \begin{itemize}
                ...
                Une liste de classes \href{https://docs.pydantic.dev/latest/}{Pydantic} pour stocker les données du formulaire suffira.
                \item Les boutons doivent être fonctionnels.
            \end{itemize}
            \column{0.4\textwidth}
            \centering
    ...
    \begin{frame}{FastAPI, définition\footnote{\url{https://fastapi.tiangolo.com/}}}
        \textbf{FastAPI} est un framework Web moderne pour Python pour créer des APIs REST.

        D’après la doc officielle~:
        \begin{itemize}
            \item \textbf{Rapide}~: très hautes performances, comparable à Node.js et Go, grâce à Starlette (serveur ASGI \url{https://starlette.dev/}) et Pydantic (validation de donnée \url{https://docs.pydantic.dev}).
            \item \textbf{Rapide à développer}~: développement 2 à 3 fois plus rapide.
            \item \textbf{Moins d’erreurs}~: validation automatique des données grâce à Pydantic.
            \item \textbf{Docs automatiques}~: Swagger UI (/docs) et ReDoc (/redoc).
            \item \textbf{Prêt pour la prod}~: basé sur des standards ouverts (OpenAPI, JSON Schema, \ldots).
        \end{itemize}
    \end{frame}

    \begin{frame}[fragile]{Exemple minimal}
        \begin{lstlisting}[language=Java,basicstyle=\tiny\ttfamily]
...
@app.get("/")
def read_root():
    return {"message": "Bonjour FastAPI"} # Un dictionnaire devient un JSON!
        \end{lstlisting}
        \begin{itemize}
            \item On crée une instance \texttt{FastAPI()}.
            \item On déclare une route avec le décorateur \texttt{@app.get("/")}.
            \item La doc interactive est disponible automatiquement.
            \item Vient avec un serveur de production, \href{https://uvicorn.dev/}{uvicorn}, si on l'installe avec la commande \lstinline{pip install fastapi[all]}.
            \item Permet d'avoir des routes asynchrones (coroutines) avec \lstinline{async def}\footnote{FastAPI, Asynchronous Code, \url{https://fastapi.tiangolo.com/async/\#asynchronous-code}}.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Les Frameworks}{Framework back-end, exercices}
        \begin{columns}
            \column{0.6\textwidth}
            Exercice \execcounterdispinc{}, avec Django~:
            \begin{itemize}
                \item Installer et faire tourner Django dans un environnement virtuel.
                À l'image du tutoriel \url{https://docs.djangoproject.com/fr/5.2/intro/tutorial01/}.
                \item À la différence du tutoriel, l'app sera un système de gestion de parkings.
                \item Il localise l'utilisateur se connectant, lui indique sa géolocalisation sur la page d'accueil.
                Elle pourra plus tard être utilisée pour sélectionner un parking à proximité.
            \end{itemize}
            \column{0.4\textwidth}
            \centering
            \includegraphics[width=4cm]{image/django-drawing} \\ \href{https://www.flickr.com/photos/espinosa_rosique/16749672584}{Django Reinhardt by A. Espinosa}, \href{https://creativecommons.org/licenses/by-nc/2.0/}{CC BY-NC 2.0}
        \end{columns}
    \end{frame}
